...
class StaffWalkInController {
  ...
  static async rescheduleWalkInTimeSlot(req, res) {
    try {
      ...
      try {
        console.log('🔍 Starting notification process for booking:', bookingId);
        
        const booking = await WalkInBooking.getWalkInBookingByIdWithServices(bookingId);
        console.log('🔍 Retrieved booking for notification:', {
          id: booking?.id,
        ...
        if (booking && booking.owner_id) {
          ...
          console.log('🔍 Creating notification with:', { title, message, owner_id: booking.owner_id });
          ...
          console.log('✅ Database notification created:', dbNotification);

          // Send real-time notification
          console.log('🔍 Sending real-time notification to user:', booking.owner_id);
          sendNotificationToUser(booking.owner_id, {
            notification: {
              id: dbNotification.id,
              title,
          ...
          console.error('❌ No booking or owner_id found:', { booking: !!booking, owner_id: booking?.owner_id });
        }
      } catch (notificationError) {
  ...
  static async startGroomingSession(req, res) {
    try {
      ...
      try {
        ...
        if (booking && booking.owner_id) {
          console.log('📨 Sending notification to owner ID:', booking.owner_id);
          ...
          console.error('❌ Booking not found or missing owner_id:', { booking });
        }
      } catch (notificationError) {
...
static async getHistoryWalkInBookings(req, res) {
  try {
    ...
    console.error('❌ Error getting historical bookings:', error);
    res.status(500).json({ 
      success: false,
